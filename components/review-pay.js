...
const ReviewPayment = ({ navigation, amount, cardSelect, cardSelected }) => {
    ...
    const [pFee, setPfee] = useState(null);
    const [plusAble, setplusAble] = useState(0);
    ...
    const getPFee = async() => {
        await axiosconfig.get(`admin/processing_fee_get`,
            {
                headers: {
                    Authorization: 'Bearer ' + myContext.userToken //the token is a variable which holds the token
                }
            ...
            if(res.data?.type == '%'){
                ...
                let per = Number(res.data?.fee) / 100 * amo;
                setplusAble(per)
            }else{
                setplusAble(Number(res.data?.fee))
            }
        }).catch((err) => {
    ...
    return (
        <View style={{ padding: 20 }}>
            ...
            <Text style={{ marginTop: 5, fontSize: moderateScale(20), color: '#000', fontFamily: 'Gilroy-Bold' }}>{pFee?.type == '$' ? pFee?.type : null}{Number(pFee?.fee)}{pFee?.type == '%' ? pFee?.type : null}</Text>
